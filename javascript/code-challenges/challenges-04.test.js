...
const updateAnimal = (arr, callback) => {
  let animalUpdateArray = [];
  arr.forEach(value => animalUpdateArray.push(callback(value)));
  return animalUpdateArray;
};
...
const sortNumbers = (arr) => {
  let lexicalOrder = function (x,y) {
    return x - y;
  };
  return arr.sort(lexicalOrder);
};
...
const sortBackwards = (arr) => {
  let reverseNum = function (x, y) {
    return y - x;
  };
  return arr.sort(reverseNum);
};
...
const sortByPrice = (arr) => {
  let priceCriteria = ({price:x},{price:y}) => x-y;
  return arr.sort(priceCriteria);
};
