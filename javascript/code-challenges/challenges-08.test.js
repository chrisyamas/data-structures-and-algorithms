...
const sortByChildren = (charArray) => {
  return charArray.sort((x,y) => x.house.localeCompare(y.house),{}).sort((x,y) => x.children.length - y.children.length,{});
};
...
const containsW = (str) => {
  ...
  return str.match(regexGlobal) ? true : false;
};
...
const isNum = (input) => {
  const regex = /\d/;
  return input.toString().match(regex) ? true : false;
};
...
const containsWorld = (input) => {
  const regexGlobal = /\b(world)\b/g;
  return input.toString().match(regexGlobal) ? true : false;
};
...
const isCapitalized = (str) => {
  const regexGlobal = /\b[A-Z]\w+/g;
  const matchRegex = str.match(regexGlobal);
  return matchRegex !== null ? matchRegex : [];
};
