...
const maxInArray = (arr) => {
  let findMax = (x,y) => Math.abs(x<y ? y:x);
  return arr.reduce(findMax);
};
...
const checkValues = (obj, value) => {
  let isValueInObj = Object.values(obj).includes(value);
  return isValueInObj;
};
...
const updateNumbers = (obj) => {
  let nameNumberJoin = Object.entries(obj).map(value => value.join(': '));
  return nameNumberJoin;
};
...
const getHouses = (arr) => {
  let houses = [];
  let pushHouses = value => {houses.push(value.house);};
  arr.forEach(pushHouses);
  return houses;
};
...
const hasChildrenValues = (arr, character) => {
  let charsWithChildren = Array.isArray(Object.values(arr.filter(value => value.name===character)[0])[2]);
  return charsWithChildren;
};
