...
const screenForNames = (arr) => {
  let nameScreenRegex=/^(Mr.|Mrs.|Ms.|Dr.)\s[A-Za-z]+/;
  return arr.filter(value => nameScreenRegex.test(value));
};
...
const toTitleCase = (arr) => {
  let capitalizedArr=arr.map(value => value.charAt(0).toUpperCase()+value.substring(1));
...
let biggerThanLuke = (arr) => {
  let lusky=arr.find(value => value.name === 'Luke Skywalker');
  let tallPeeps=arr.filter(value => parseInt(value.mass)>parseInt(lusky.mass)).map(value => value.name).join(' - ');
  return tallPeeps;
};
...
const sortBy = (property, arr) => {
  let propertySort = arr.sort((x,y) => x[property]<y[property] ? -1:1);
  return propertySort;
};
...
const detectTicTacToeWin = (board) => {
  let checkSquare = (r1, c1, r2, c2, r3, c3) => {
    return board[r1][c1] !== '' &&
    board[r1][c1]===board[r2][c2] &&
    board[r2][c2]===board[r3][c3];
  };
  if(checkSquare(0, 0, 1, 0, 0, 2)) return true;
  if(checkSquare(1, 0, 1, 1, 1, 2)) return true;
  if(checkSquare(2, 0, 2, 1, 2, 2)) return true;

  if(checkSquare(0, 0, 1, 0, 2, 0)) return true;
  if(checkSquare(0, 1, 1, 1, 2, 1)) return true;
  if(checkSquare(0, 2, 1, 2, 2, 2)) return true;

  if(checkSquare(0, 0, 1, 1, 2, 2)) return true;
  if(checkSquare(0, 2, 1, 1, 2, 0)) return true;
  return false;
};
