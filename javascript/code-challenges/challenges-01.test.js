...
const addOne = (arr) => {
  let newArray = [];
  arr.forEach((arrVal) => (newArray.push(arrVal+1)));
  return newArray;
};
...
const addExclamation = (arr) => {
  let exclamArr = [];
  arr.forEach((arrVal) => (exclamArr.push(`${arrVal}!`)));
  return exclamArr;
};
...
const allUpperCase = (arr) => {
  let uppCasArr = [];
  arr.forEach((arrVal) => (uppCasArr.push(arrVal.toUpperCase())));
  return uppCasArr;
};
...
const speaker = (words, callback) => {
  let speakArr = [];
  words.forEach((word) => speakArr.push(callback(word)));
  return speakArr;
};
