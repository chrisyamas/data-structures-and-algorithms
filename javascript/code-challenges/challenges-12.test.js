...
const replaceZeros = (string) => {
  let zerosRegex = /[0]/g;
  return string.replace(zerosRegex,'zero');
};
...
const validateWord = (word) => {
  let validRegex = /^[a-zA-Z]{5,10}$/;
  return validRegex.test(word);
};
...
const hasNumber = (string) => {
  let numdigRegex = /[a-zA-Z]\d/;
  return numdigRegex.test(string);
};
...
const validateEmail = (email) => {
  let emailValidRegex = /^[a-zA-Z0-9]+\.?[a-zA-Z0-9]+@[a-zA-Z0-9]+\.(net|com|org)\b/;
...
const validatePhoneNumber = (phoneNumber) => {
  let phoneValidRegex = /^(\(\d{3}\)|\d{3}) ?-?\d{3} ?-?\d{4}$/;
  return phoneValidRegex.test(phoneNumber);
};
