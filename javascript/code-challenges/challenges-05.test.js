...
const toLastNames = people => {
  let fullName = people.map((value) => `${value.firstName} ${value.lastName}`);
  return fullName;
};
...
const addValues = (arr) => {
  let initial = 0;
  let valueAddArray = arr.reduce((previous,current) => previous+current, initial);
  return valueAddArray;
};
...
const addPurchases = (arr) => {
  let totalPurchased = arr.reduce((x,y) => x+y.purchasePrice,0);
...
const countNumberOfElements = (arr) => {
  let elementsCount = arr.reduce(x => x+1);
...
const returnNames = (arr) => {
  let characterNames = arr.reduce((namesArr, character) => {
...
const reversedString = (str) => {
  let flipItBaby = str.split('').reduce((x,y) => y+x, '');
  return flipItBaby;
};
