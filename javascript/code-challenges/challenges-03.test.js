...
const containsAnd = (arr) => {
  let andArray = arr.filter(value => value.includes('and'));
  return andArray;
};
...
const oddValues = (arr) => {
  let oddDetermine = value => value % 2 === 1;
  let oddArray = arr.filter(oddDetermine);
  return oddArray;
};
...
const notInFirstArray = (forbiddenValues, arr) => {
  let valuesCriteria = value => !forbiddenValues.includes(value);
  let secondArray = arr.filter(valuesCriteria);
  return secondArray;
};
